...
interface ItemOrderCategoryProps {
    ...
    <TouchableOpacity onPress={onDetail} style={[Theme.flex1]}>
            ...
            <View style={[Theme.flex1, {paddingBottom: 8}]}>
              ...
              <View style={[Theme.flexRow]}>
                <Text
                  ...
                  {(
                    getPriceVoucher(item?.product) || item?.product?.price
                  )?.formatPrice()}
                  đ
                </Text>
                {!!getPriceVoucher(item?.product) && (
      ...
      (item?.status == 'COMPLAIN' || item?.status == 'RECEIVED') ? (
        <ButtonBorder
          ...
          onPress={onRating}
          height={42}
          style={[Theme.flex1, {marginTop: 10}]}
...
export default ItemOrderCategory;
